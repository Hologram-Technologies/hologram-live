...
- `.holo` capability admission: canonical requests are distinct from trusted effective grants; the exact content-addressed zero-byte sentinel from early Live archives remains a deny-all compatibility value while every nonempty object stays strict; the default local baseline has no storage/channel/network authority, explicit development grants are restricted to direct files or loopback service configuration, denial occurs before provider preparation, and durable audit rows plus run/resident results report non-secret request/grant identities, relation, principal, trusted source, and outcome across CLI, JSON/HTTP, and Protobuf/gRPC.
