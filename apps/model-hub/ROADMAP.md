...
| G3 | Drop-in `HF_ENDPOINT` and `ollama pull` | Live: redirects to a healthy source; two client lines and Ollama measured; whole-model failover measured with Hugging Face blackholed (ModelScope + IPFS, and IPFS alone) | P0 | Upstream the hub shim (manifest codec, tree, resolve with Range and ETag) as small PRs; serve read-only on the hub |
