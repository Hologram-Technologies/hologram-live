...
docker run --rm \
  ...
  -e BASE=/ \
  node:22-alpine sh -c 'npm ci --no-fund --no-audit >/dev/null && node scripts/data.mjs --limit 500 && node scripts/registry.mjs && node scripts/lint-tokens.mjs && node build.mjs'
