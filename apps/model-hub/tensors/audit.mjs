...
for (const [repo, r] of Object.entries(root.models)) {
  ...
  if (r.provenance) for (const row of JSON.parse(ref(r.provenance, `${repo} provenance`)).rows) { if (!K.test(row[0]) || !K.test(row[2])) out.notKappa.push(`${repo} provenance row ${row[0]}`); }
