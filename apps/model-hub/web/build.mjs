...
const page = ({ title, description, body, search = false }) => sandbox(`<!doctype html>
<html lang="en" class="dark" data-theme="dark" data-wallpaper="alps" data-base="${base}" data-target="${HOLO ? "holo" : "site"}">
<head>
<meta charset="utf-8">
...
function modelPage(m, files, ov, readme) {
  ...
  if (files) {
    ...
    filesPanel = HOLO ? `<p class="progress" id="node-status" role="status">${files.files.length} files, ${R.bytes(total)}. Pull stores every file on this node, each checked against its address.</p>
    <div class="scroll"><table id="files">
      ...
      <tbody>${rows}</tbody>
    </table></div>` : `<div class="section-head">
