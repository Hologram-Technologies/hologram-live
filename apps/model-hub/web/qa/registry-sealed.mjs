...
//
// Two reasons, and the second is the sharp one. The hub loads no third-party asset anywhere, so this
// page should not be the exception. And a per-card icon fetched from a CDN would tell that CDN which
// registry entries each visitor scrolled past: a behavioural log of our readers' interest, handed to
// someone else, on a page whose entire claim is that you can check what you were given. A hasher loaded
// from a CDN is worse still, because then the verification itself is on loan.
//
...
if (fail.length) { console.error("registry page is not sealed:\n  " + fail.join("\n  ")); process.exit(1); }
console.log(`registry page sealed: ${data.images.length} rows, ${icons} vendored covers, no external load`);
