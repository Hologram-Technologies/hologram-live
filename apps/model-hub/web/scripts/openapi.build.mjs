...
function document(server, evidence) {
  ...
  spec.paths["/api/hub/health"] = {
    get: {
      ...
      summary: "Which byte sources are up",
  ...
  spec.paths["/api/models"] = {
    get: {
      ...
      description: "Hugging Face's list shape, so `HfApi.list_models` works unchanged, plus a `hologram` block on every row carrying the facts Hugging Face has no field for: the index address of the model, how many files it has, how many bytes of weights, and which sources hold them. A search costs a few hundred bytes; reading the whole catalog object costs 364 KB, so search here rather than there.",
      parameters: [
  ...
  spec.paths["/via/{source}/{owner}/{name}/resolve/{revision}/{path}"] = {
    get: {
      ...
      description: "The caller states the policy and the hub executes it: `/via/ipfs/...` serves from IPFS even when Hugging Face is up, and refuses rather than falling back. The prefix works in front of any Models or Files route, not only this one.",
      parameters: [SOURCE, OWNER, NAME, REVISION, FILEPATH],
      responses: {
        302: { description: "Redirect to the named source.", headers: { location: { description: "Where the bytes are.", schema: { type: "string", format: "uri" } }, "x-hub-source": { description: "The source you named.", schema: { type: "string" } } } },
        ...
        404: hubError(404, "Unknown model, revision or file; or the named source does not hold this file.", body("resolve.entry.missing")),
        405: READ_ONLY[405],
      },
  ...
  return {
    ...
    HologramFacts: {
      ...
      properties: {
        ...
        sources: { type: "array", items: { type: "string" }, description: "Which sources hold these bytes today." },
      },
    },
    ModelRow: {
      type: "object",
    ...
    TreeEntry: {
      ...
      properties: {
        ...
        oid: { ...sha256, description: "The SHA-256 of the file's bytes. This is the expected hash." },
        size: { type: "integer" },
        path: { type: "string" },
        lfs: { type: "object", description: "Present on large files, for clients that branch on it.", properties: { oid: sha256, size: { type: "integer" }, pointerSize: { type: "integer" } } },
      },
    },
    Refs: {
      type: "object",
    ...
    HubHealth: {
      ...
      properties: {
        sources: { type: "object", additionalProperties: { type: "object", required: ["ok", "checked"], properties: { ok: { type: "boolean" }, checked: { type: "string", format: "date-time" }, reason: { type: "string", description: "Why the hub believes it, in one word." } } } },
