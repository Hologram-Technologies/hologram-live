...
const hubError = (status, description, example) => ({
  description,
  headers: {
    "x-error-code": { description: "A closed vocabulary: ReadOnly, GatedRepo, RepoNotFound, RevisionNotFound, EntryNotFound, NotFound, HubError.", schema: { type: "string" } },
...
function document(server, evidence) {
  ...
  spec.paths["/api/models"] = {
    get: {
      ...
      description: [
        ...
        "",
        "**This route lists the browse set, not the whole index.** `X-Total-Count` is the size of what is",
        "searchable, and the catalog object the descriptor points at addresses more models than that — around 575",
        "against 416 at the time of writing. A model missing from these results may still be fully served by",
        "`getModel`, `listModelFiles`, `resolveFile`, the registry and the IPFS pins; `BAAI/bge-base-en-v1.5` is one",
        "such today. If a name matters to you, ask for it directly rather than concluding from a search that the",
        "hub does not have it.",
        "",
        "Query parameters are advisory: an out-of-range `limit` or an unknown `sort` falls back to the default",
  ...
  spec.paths["/via/{source}/{owner}/{name}/resolve/{revision}/{path}"] = {
    get: {
      ...
      summary: "A URL for one file, preferring the source you name",
      description: [
        "The caller states a preference and the hub tries to honour it: `/via/ipfs/...` serves from IPFS even when",
        "Hugging Face is up. The prefix works in front of any Models or Files route, not only this one.",
        "",
        "**It is a preference, not a guarantee, and it fails open.** When the named source does not hold the file the",
        "hub falls back to whichever source does, with no error — and an unknown source name is accepted and ignored",
        "rather than refused. `/via/modelscope/BAAI/bge-base-en-v1.5/...` redirects to huggingface.co today, and so",
        "does `/via/bogus/...`.",
        "",
        "So the only reliable signal of who actually served you is the `X-Hub-Source` response header. If you pin a",
        "source for availability, jurisdiction, or to compare two sources against each other, read that header and",
        "treat a value other than the one you asked for as a miss. Comparing two fetches without checking it can",
        "mean comparing the same host to itself.",
      ].join("\n"),
      ...
      responses: {
        ...
        404: hubError(404, "Unknown model, revision or file. Note that a source that does not hold the file is *not* a 404: the hub falls back silently, so check `X-Hub-Source`. A malformed source segment, such as one with capitals, is refused by the edge as a bare 404 with an empty body rather than in this shape.", body("resolve.entry.missing")),
        405: READ_ONLY[405],
      },
