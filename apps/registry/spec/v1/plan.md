...
| 028 | Registry mode network surface and the admin transport. Public listener: `/v2/`, `/`, `/healthz`, `/docs`, `/openapi.json`. System API behind a token generated at start and written to `<root>/live/state/admin-token` (mode 0600). gRPC on a second, loopback-only, always-plain listener (`127.0.0.1:5001`), so `docker exec registry hologram oci verify` works with TLS on | P5 T3, extended P6 T3 | As stated. Closes FR-021 |
