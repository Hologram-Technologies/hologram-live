...
$BuildArguments = @("build", "--manifest-path", (Join-Path $Root "Cargo.toml"), "--release", "--locked", "--package", "hologram-live", "--bin", "hologram")
if ($env:HOLOGRAM_CARGO_FEATURES) {
    $BuildArguments += @("--features", $env:HOLOGRAM_CARGO_FEATURES)
}
