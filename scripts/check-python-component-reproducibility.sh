...
def identity(report, archive_path):
    ...
    if patch_set.get("determinism_contract") != "hologram:componentizer/preinitialization-determinism@2":
        raise ValueError("compile report does not contain the deterministic componentizer contract")
    if patch_set.get("release_tag") != "componentizer-v0.25.0-hologram.2":
