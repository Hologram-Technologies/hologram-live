...
Chat and model management call an `InferenceEngine` trait selected by `[inference].engine` in `live.toml`. Engines normally execute out of process. Five engines ship:
...
- `vllm` — uses vLLM's OpenAI-compatible completion and model APIs, including native SSE streaming.
- `llamacpp` — an off-by-default, in-process GGUF engine accepted as a scoped exception by ADR 033.
...
- ADR 033 amends the original no-in-process rule only for binaries explicitly built with `llamacpp`; the trait boundary and default build remain unchanged.
