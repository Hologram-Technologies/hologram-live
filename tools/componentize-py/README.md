...
and `PATCHSET.sha256` under a `componentizer-v*` release tag. The compiler must
pin an exact release URL and checksum for every supported host; it must never
select these assets by a mutable release name.
